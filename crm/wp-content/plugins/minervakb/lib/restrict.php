...
class MinervaKB_Restrict {
    ...
    /**
     * Gets restricted articles query param to use in WP_Query (inversion of allowed)
     * @return array
     */
    public function get_restricted_article_ids_query() {
        if ($this->is_current_user_admin() || apply_filters('minerva_restrict_access_allowed', false)) {
            return array(); // admins can watch all content
        }

        $allowed_ids = $this->get_allowed_article_ids_for_user();
        $restricted_ids = array();

        $all_articles = MKB_DbModel::get_all_article_ids();

        if (sizeof($all_articles)) {
            foreach($all_articles as $article) {
                if (in_array($article->ID, $allowed_ids)) { continue; }

                $restricted_ids[]= $article->ID;
            }
        }
	...
	public static function invalidate_restriction_cache($rebuild = true) {
        ...
        if (!empty($languages)) {
            foreach($languages as $lang) {
                delete_transient($lang['language_code'] . '_' . self::$ARTICLES_CACHE_KEY);
                delete_transient($lang['language_code'] . '_' . self::$TOPICS_CACHE_KEY);
            }
        }
	}
	...
	private function get_cached_allowed_entries_ids_list($type, $transient_key, &$local_cache_key, $callback, $roles = null) {
		...
		if (defined('ICL_LANGUAGE_CODE') && ICL_LANGUAGE_CODE) {
		    $transient_key = ICL_LANGUAGE_CODE . '_' . $transient_key;
        }
